...
public class searchtools {
    ...
    */
    private void search(File topDirectory, List res, int objectType) throws SQLException {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (list[i].isDirectory()) {
                ...
                if (objectType != FILES && accept(list[i].getName())) {
                    ...
                    directoriesNumber++;
                    res.add(list[i]);
                    Controller.temp.add(list[i]);
                    query.WriteTB(Controller.ServerName, list[i].toString());
                    Controller.count++;
                }
